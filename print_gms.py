...
class MIDISettings:
	...
	def __str__(self):
		...
		return																								\
			...
			f'{self._setting_labels[8]}{self._off_on_labels[self._sysex_xmit]}\n' +							\
			f'{self._setting_labels[9]}{self._off_on_labels[self._xmit_clock]}\n' +							\
...
def print_genos_midi_settings(genos_file, analyze=False):
	...
	print(f'Setting:    {genos_file_name}', file=console_out)
	print(settings)
